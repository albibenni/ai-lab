...
async function runEval(): Promise<EvalResult[]> {
  ...
  for (const testCase of dataset) {
    ...
    console.log(
      `  → ${passed ? "✅" : "❌"} Expected: ${testCase.expected}, Got: ${category} (confidence: ${confidence.toFixed(2)})`,
    );
  }
...
function calculateMetrics(results: EvalResult[]): Metrics {
  ...
  const byDifficultyMetrics: Record<
    string,
...
// ============================================
export async function main_eval() {
  ...
  for (const [difficulty, stats] of Object.entries(metrics.byDifficulty)) {
    console.log(
      `  ${difficulty}: ${(stats.accuracy * 100).toFixed(1)}% (${stats.total} tests)`,
    );
  }
